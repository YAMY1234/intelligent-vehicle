...
def ReturnRequest(dl_ord, mode):
    ...
    sql = "select orderod,ordernum,orderlist,secondstime,starttime,charternum from " + table_name + " where date(starttime)=date('" + str(
        start_time.date()) + "')"
    cur.execute(sql)
    ...
    mintimesel = start_time.hour * 12001
    mintimesat = start_time.hour * 12001

    # 运力简单判断，1200取值应为最不利情况调车+本次行程时间
    for raw in results:
        start_time_e = raw[4]
        second_e = raw[3]
        oidnum_e = raw[1]
        ...
        if (second_e == second_order and charternum == 0):
            ...
            if ((od_e.split("-")[0] == ostart or od_e.split("-")[1] == dstart) and addflag == False):
                mintimesat = 0
                breakout = findex
        if (abs(second_order - second_e) <= 1200 and second_e != second_order and charternum == 0):
            totalnum += oidnum_e
            coutDingdan += math.ceil(oidnum_e / 3)
            if (od_e == ostart + "-" + dstart and mintimesel > abs(second_order - second_e) and addflag == False):
                breakout = findex
                mintimesel = second_order - second_e
            if ((ostart in od_e or dstart in od_e) and mintimesat > abs(
                    second_order - second_e) and mintimesel == start_time.hour * 12001 and addflag == False):
    ...
    if (start_time_e != None and samecaraddflag == False):
        if (totalnum + e > maxzuoweibuffer):
            ...
            if (coutDingdan >= maxbusnum and breakout == -1):
                ...
                if (coutDingdan >= maxbusnum and breakout != -1 and mintimesel != start_time.hour * 12001):
                    cur.close()
                    conn.close()
    ...
    if (breakout != -1 and addflag == True):
        ...
        sql = "update " + table_name + " set orderlist='" + str(oidlist_e) + "', ordernum=" + str(
            oidnum_e) + " where orderod='" + str(od_e) + "' and date(starttime)=date('" + str(
            start_time.date()) + "') and secondstime=" + str(second_order) + " and charternum=0"
        cur.execute(sql)
        conn.commit()
    else:
        sql = "insert into " + table_name + " values (str_to_date('" + str(
            d) + "','%Y-%m-%d %H:%i:%s'),'" + a + "'," + str(e) + ",'" + ostart + "-" + dstart + "'," + str(
            second_order) + ",'" + dt.datetime.strftime(dt.datetime.now(), '%Y-%m-%d %H:%M:%S') + "','-1',0)"
        cur.execute(sql)
        conn.commit()
